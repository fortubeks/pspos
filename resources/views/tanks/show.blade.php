...
<div>
    ...
    <div class="card">
        <div class="card-header pb-0 px-3">
            <h6 class="mb-0">{{ __('Tank') }}</h6>
        </div>
        <div class="card-body pt-4 p-3">
            <form action="{{url('/tanks/'.$tank->id)}}" method="POST" role="form text-left">
                @csrf
                @if($errors->any())
                ...
                <div class="row">
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <div class="@error('qty')border border-danger rounded-3 @enderror">
                                <input class="form-control" value="{{$tank->name}}" type="text" name="name">
                                    @error('name')
                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <div class="@error('product')border border-danger rounded-3 @enderror">
                                <select class="form-select form-control" id="product" data-product="{{$tank->product->id}}" name="product">
                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            <label for="user-name" class="form-control-label">{{ __('Balance') }}</label>
                            <div class="@error('qty')border border-danger rounded-3 @enderror">
                                <input class="form-control" type="number" inputmode="decimal" value="{{$tank->balance}}" min="0" step="any" placeholder="Dipping Reading" name="balance">
                                    @error('name')
                                        <p class="text-danger text-xs mt-2">{{ $message }}</p>
                                    @enderror
                            </div>
